...
//        var_dump($this->fce->T_DATA->row);
    ...
    public function COSTCENTER_GETLIST ($user, $debug = false) {
...
//
    ...
    public function getPROpen($data = false, $by = 'plant') {
        ...
        $availabledata = array('PRNO', 'PRITEM', 'DOCTYPE', 'DOC_CAT', 'DEL', 'NOMAT', 'PLANT', 'DECMAT', 'QUANTOPEN', 'POQUANTITY', 'PRQUANTITY', 'HANDQUANTITY', 'RATAGI', 'MAXGI', 'LASTGI', 'UOM', 'REALDATE', 'POSTDATE', 'MAX_GI_YEAR', 'MAX_YEAR_GI', 'NETPRICE', 'PER', 'CURR', 'MATGROUP', 'PORG', 'REQUESTIONER', 'PGRP', 'MRPC', 'DDATE', 'BSMNG', 'PACKNO', 'STATU');
        $itTampung = array();
        if ($this->fce->GetStatus() == SAPRFC_OK) {
            $this->fce->IT_DATA->Reset();
            while ($this->fce->IT_DATA->Next()) {
    ...
    */
	...
	public function creategr($KODE_DETAIL_SHIPMENT,$PO_NO,$QTY,$user,$DATE) {
        $debug = true;
        ...
        //foreach ($grheader as $value) {
            ...
            $this->fce->GOODSMVT_HEADER['PSTNG_DATE'] = $DATE;
            $this->fce->GOODSMVT_HEADER['DOC_DATE'] = date("Ymd");
